...
Axis.prototype = {
	...
	update: function (newOptions, redraw) {
		...
		newOptions = chart.options[this.xOrY + 'Axis'][this.options.cms_data] = merge(this.userOptions, newOptions);
	...
	remove: function (redraw) {
		...
		chart.options[key].splice(this.options.cms_data, 1);
		...
		each(chartSeries, function (series) {
			...
			if (isString(linkedTo)) {
				if (linkedTo === ':previous') {
					linkedTo = chart.series[series.cms_data - 1];
				} else {
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		stableSort(chartSeries, function (a, b) {
			return pick(a.options.cms_data, a._i) - pick(b.options.cms_data, a._i);
		});
		...
		if (series.isCartesian) {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the chart's axis objects
					...
					if ((seriesOptions[AXIS] === axisOptions.cms_data) ||
							(seriesOptions[AXIS] !== UNDEFINED && seriesOptions[AXIS] === axisOptions.id) ||
							(seriesOptions[AXIS] === UNDEFINED && axisOptions.cms_data === 0)) {
	...
	setStackedPoints: function () {
		...
		for (i = 0; i < yDataLength; i++) {
			...
			stack = stacks[key][x];
			stack.points[series.cms_data] = [stack.cum || 0];
			...
			stack.points[series.cms_data].push(stack.cum);
	...
	setPercentStacks: function () {
		...
		each([stackKey, '-' + stackKey], function (key) {
			...
			while (i--) {
				...
				pointExtremes = stack && stack.points[series.cms_data];
				if (pointExtremes) {
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			if (stacking && series.visible && stack && stack[xValue]) {
				...
				stackValues = pointStack.points[series.cms_data];
